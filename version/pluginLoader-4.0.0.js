...
async function messageCreate(client, message) {
    ...
    for (let index = 0; index < plugin.length; index++) {
        try {
            if (Reload[plugin[index]].Plugin.DHL != undefined && Reload[plugin[index]].Plugin.DHL == false) {
    ...
    try {
        if (message.content.startsWith(Prefix + "permission")) {
            ...
            if (message.content == Prefix + "plugin reload") {
                ...
                if (Reload[args[0] + ".js"] != undefined) {
                    if (Reload[args[0] + ".js"].Plugin.DHL != undefined && Reload[args[0] + ".js"].Plugin.DHL == false) {
                        try {
                            Reload[args[0] + ".js"] = reload('../Plugin/' + args[0] + ".js")
                            await message.reply(await embed(`⚠️ 已重新加載 ${args[0]} 僅對關閉 DHL 的插件有效`))
                        } catch (error) {
                            log(`Error >> ${plugin[index]} 運行出錯 請向 插件 作者聯繫\n錯誤碼:\n${error}`)
                        }
                    } else {
            ...
            if (!down.state) {
                ...
                try {
                    ...
                    if (fun.Plugin == undefined || fun.Plugin.Events == undefined || fun.Plugin.Commands == undefined || fun.Plugin.version == undefined || fun.Plugin.name == undefined || fun.Plugin.author == undefined || fun.Plugin.depends == undefined || fun.Plugin.depends.pluginLoader == undefined) {
                        ...
                        if (fun.Plugin.DHL == undefined || fun.Plugin.DHL == true) {
                            msg = msg + "🟦 啟用 DHL 模式\n"
                        } else {
                            msg = msg + "🟨 未啟用 DHL 模式\n"
                        }
                        ...
                        for (let index = 0; index < Object.keys(fun.Plugin.depends).length; index++) {
                            if (Object.keys(fun.Plugin.depends)[index] != "pluginLoader") {
                                console.log(Object.keys(fun.Plugin.depends)[index])
                                if (!fs.readdirSync(Path + "/Plugin/").includes(Object.keys(fun.Plugin.depends)[index] + ".js")) {

                ...
                try {
                    ...
                    if (fun.Plugin == undefined || fun.Plugin.Events == undefined || fun.Plugin.Commands == undefined || fun.Plugin.version == undefined || fun.Plugin.name == undefined || fun.Plugin.author == undefined || fun.Plugin.depends == undefined || fun.Plugin.depends.pluginLoader == undefined) {
                        ...
                        for (let index = 0; index < Object.keys(fun.Plugin.depends).length; index++) {
                            ...
                            await channels.send(`$plugin i ${Object.keys(fun.Plugin.depends)[index]}`)
                            msg = msg + `⏳ 正在安裝 ${Object.keys(fun.Plugin.depends)[index]} 依賴插件...\n`
                            edit(client, MSG.channel.id, MSG.id, await embed(msg))
                        }
                        msg = msg + "⏳ 註冊 事件監聽...\n⏳ 註冊 插件訊息...\n⏳ 註冊 插件指令...\n"
                        edit(client, MSG.channel.id, MSG.id, await embed(msg))
                        ...
                        for (let index = 0; index < Object.keys(fun.Plugin.depends).length; index++) {
                            ...
                            if (!await compatible(fun.Plugin.depends[Object.keys(fun.Plugin.depends)[index]], cache.Plugin.version)) {
                                msg = msg + `🟨 插件 不兼容 當前 ${Object.keys(fun.Plugin.depends)[index]} 最低: ${fun.Plugin.depends[Object.keys(fun.Plugin.depends)[index]]} 當前: ${cache.Plugin.version}...\n`
                                edit(client, MSG.channel.id, MSG.id, await embed(msg))
                                fail = 1
                            }
                        }
                        ...
                        // if (!await compatible(fun.Plugin.depends.pluginLoader, Plugin.version)) {
...
async function compatible(ver, Ver) {

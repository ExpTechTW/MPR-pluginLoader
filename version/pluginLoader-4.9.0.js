...
async function log(msg, client) {
    ...
    let Now = now.getFullYear() +
        ...
        ":" + now.getMinutes() +
        ":" + now.getSeconds()
    if (!fs.existsSync(Path + "/Data/Log")) {
        fs.mkdirSync(Path + "/Data/Log")
    }
    if (!fs.existsSync(Path + `/Data/Log/${(now.getMonth() + 1) + "-" + now.getDate()}_log.txt`)) {
        fs.writeFileSync(Path + `/Data/Log/${(now.getMonth() + 1) + "-" + now.getDate()}_log.txt`, "", 'utf8')
    }
    let Log = fs.readFileSync(Path + `/Data/Log/${(now.getMonth() + 1) + "-" + now.getDate()}_log.txt`).toString()
    Log = `[${Now}] ${msg} \n` + Log
    fs.writeFileSync(Path + `/Data/Log/${(now.getMonth() + 1) + "-" + now.getDate()}_log.txt`, Log, 'utf8')
}

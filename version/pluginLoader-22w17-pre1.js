...
async function plugin(client, message) {
    try {
        ...
        } else if (await permission(message.author.id) < 3) {
            await message.reply(await embed(`權限不足`))
            return
            ...
            if (!down.state) {
                ...
                try {
                    ...
                    if (fun.Info == undefined || fun.Event == undefined || fun.Commands == undefined) {
                        ...
                        if (fun.Info.depends != undefined && fun.Info.depends.lenght != 0) {
                            msg = msg + "🟦 正在安裝 依賴插件...\n"
                            edit(client, MSG.channel.id, MSG.id, await embed(msg))
                            for (let index = 0; index < fun.Info.depends.length; index++) {
                                let channels = await client.channels.cache.get(MSG.channel.id)
                                await channels.send(`$plugin i ${fun.Info.depends[index]}`)
                            }
                        }
                        msg = msg + "🟦 註冊 事件監聽...\n🟦 註冊 插件訊息...\n🟦 註冊 插件指令...\n"
                        edit(client, MSG.channel.id, MSG.id, await embed(msg))
